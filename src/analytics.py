...
from storage.dynamodb_storage import get_snapshot_at_or_before
from storage.serialization import snapshot_to_portfolio
...
def print_analytics(
    ...
    """Print completed-session analytics for development verification."""
    ...
    print(
        "Previous session: "
        f"{analytics.previous_session_date.strftime('%A, %B %d, %Y')}"
    )

    print(
        "Report session:   "
        f"{analytics.session_date.strftime('%A, %B %d, %Y')}"
    )

    print(
        "Return period:    "
        f"{analytics.previous_session_date} close -> "
        f"{analytics.session_date} close"
    )

    print()
    print("PERFORMANCE METHOD")
    print("=" * 65)

    if (
        analytics.performance_method
        == SNAPSHOT_METHOD
    ):
        print(
            "Historical Snapshot"
        ...
        print(
            "Snapshot Used: "
            f"{analytics.historical_snapshot_timestamp}"
        )
    else:
        print(
        ...
        if (
            analytics.historical_snapshot_available
        ):
            print(
                "Historical Snapshot: Found, "
                "but not suitable for this calculation"
            )
            print(
                "Snapshot Found: "
                f"{analytics.historical_snapshot_timestamp}"
            ...
            print(
                "Historical Snapshot: "
                "Not available for this market period"
            )
    ...
    print(
        f"Portfolio Value:          "
        f"${analytics.portfolio_value:,.2f}"
    )

    print(
        f"Previous Close Value:     "
        f"${analytics.estimated_previous_value:,.2f}"
    )

    print(
        f"Investment Change:        "
        f"${analytics.dollar_change:+,.2f}"
    )

    print(
        f"Portfolio Return:         "
        f"{analytics.portfolio_return:+.2%}"
    )

    print(
        f"{analytics.benchmark_symbol} Return:                "
        f"{analytics.benchmark_return:+.2%}"
    )

    print(
        f"Relative Performance:     "
        f"{analytics.relative_return:+.2%}"
    )

    print(
        f"Cash at Session End:      "
        f"${analytics.cash:,.2f}"
    )
    ...
    print(
        f"Trades:                   "
        f"{analytics.trade_count}"
    )

    print(
        f"Security Transfers:       "
        f"{analytics.transfer_count}"
    )

    print(
        f"Deposits:                 "
        f"${analytics.deposits:,.2f}"
    )

    print(
        f"Withdrawals:              "
        f"${analytics.withdrawals:,.2f}"
    )

    print(
        f"Net External Cash Flow:   "
        f"${analytics.net_external_cash_flow:+,.2f}"
    )

    print(
        f"Dividend / Interest:      "
        f"${analytics.dividends_and_interest:,.2f}"
    )

    print(
        f"Other Transactions:       "
        f"{analytics.other_transaction_count}"
    )
    ...
    for position in sorted_positions:
        ...
        if position.account_count > 1:
            account_note = (
                f"  "
                f"[{position.account_count} accounts]"
            )
        ...
        print(
            f"  Previous close:   "
            f"${position.previous_close:,.2f}"
        )

        print(
            f"  Session close:    "
            f"${position.latest_close:,.2f}"
        )

        print(
            f"  Return:           "
            f"{position.return_pct:+.2%}"
        )

        print(
            f"  Dollar impact:    "
            f"${position.dollar_change:+,.2f}"
        )

        print(
            f"  Portfolio weight: "
            f"{position.current_weight:.2%}"
        )
    ...
    if analytics.contributors:
        for position in (
            ...
            print(
                f"{position.symbol:<8} "
                f"${position.dollar_change:+9.2f}   "
                f"{position.portfolio_contribution:+7.2%}"
            )
    else:
        print("None")
    ...
    if analytics.detractors:
        for position in (
            analytics.detractors[:5]
        ):
            print(
                f"{position.symbol:<8} "
                f"${position.dollar_change:+9.2f}   "
    ...
    print(
        f"Largest Position: "
        f"{analytics.largest_position_weight:.2%}"
    )

    print(
        f"Top 3 Positions:   "
        f"{analytics.top_three_weight:.2%}"
    )

    print(
        f"Cash Weight:       "
        f"{analytics.cash_weight:.2%}"
    )
...
if __name__ == "__main__":
    try:
        print(
            "\nRetrieving portfolio, historical snapshots, "
            "transactions, and completed-session market data..."
        )
        ...
        print(
            "SUCCESS: Completed-session analytics generated."
        )

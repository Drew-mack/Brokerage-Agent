...
from auth import get_access_token, ReauthorizationRequired
...
def save_json(data, filename):
    """
    Save raw Schwab data locally for visual inspection.

    dev_data/ MUST remain in .gitignore.
    """
...
def iso_utc(dt):
    """
    Convert a datetime to UTC ISO-8601 format.
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "\nSUCCESS: Schwab data retrieved."
        )

        print(
            f"Accounts: "
            f"{len(snapshot['accounts'])}"
        )

        print(
            f"Portfolio securities: "
            f"{len(snapshot['portfolio_symbols'])}"
        )

        if snapshot["portfolio_symbols"]:
        ...
        complete_file = save_json(
            snapshot,
            "schwab_complete_snapshot.json",
        )
        ...
        accounts_file = save_json(
            snapshot["accounts"],
            "schwab_accounts.json",
        )

        activity_file = save_json(
            snapshot["account_activity"],
            "schwab_account_activity.json",
        )

        quotes_file = save_json(
            snapshot["market_data"]["quotes"],
            "schwab_quotes.json",
        )

        history_file = save_json(
            snapshot[
                "market_data"
            ]["price_history"],
            "schwab_price_history.json",
        )

        hours_file = save_json(
            snapshot[
                "market_data"
            ]["market_hours"],
            "schwab_market_hours.json",
        )
        ...
        print(
            "\nSUCCESS: Schwab development "
            "snapshot complete."
        )

        print(
            "\nIMPORTANT: dev_data/ contains "
            "real financial information and "

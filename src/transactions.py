...
from schwab_client import SchwabClient, iso_utc
...
def print_transaction_summary(summary):
    """
    Human-readable output for development/testing.
    """
    ...
    print(
        f"\nTransactions:          "
        f"{len(summary.transactions)}"
    )

    print(
        f"Trades:                "
        f"{len(summary.trades)}"
    )

    print(
        f"Security Transfers:    "
        f"{len(summary.transfers)}"
    )
    ...
    print(
        f"Dividend / Interest:   "
        f"${summary.dividends_and_interest:,.2f}"
    )

    print(
        f"Net External Cash Flow:"
        f" ${summary.net_external_cash_flow:,.2f}"
    )
    ...
    if summary.transactions:
        ...
        for transaction in (
            ...
            print(
                ...
                f"{symbol:<8} "
                f"${transaction.net_amount:>10,.2f}  "
...
if __name__ == "__main__":
    ...
    print(
        "\nRetrieving recent Schwab "
        "transactions..."
    )

    summary = load_transactions(

...
def print_morning_brief(
    ...
    """Print the final email-level morning brief."""

    print()
    print("=" * 65)
    print("MORNING PORTFOLIO BRIEF")
    print("=" * 65)
    ...
    print(
        f"Session:     "
        f"${brief.dollar_change:+,.2f} "
        f"({brief.portfolio_return:+.2%})"
    )

    print(
        f"{brief.benchmark_symbol}:"
        f"{' ' * max(1, 12 - len(brief.benchmark_symbol))}"
        f"{brief.benchmark_return:+.2%}"
    )

    print(
        f"vs. {brief.benchmark_symbol}:"
        f"{' ' * max(1, 8 - len(brief.benchmark_symbol))}"
    ...
    if brief.changes:
        ...
        for change in brief.changes:
            ...
            print(
                f"{change.symbol} "
                f"{change.return_pct:+.2%}"
            )

            print(
                f"Portfolio Impact: "
                f"${change.dollar_impact:+,.2f}"
            )
    ...
    if brief.advice:
        print()
        print("AGENTIC ADVICE")

        for item in brief.advice:
            print()
            print(
                item.symbol
            )
            ...
            print(
                f"{item.portfolio_weight:.1%} "
                f"of portfolio · "
                f"{item.outlook.title()}"
            )
    ...
    print()
    print(
        f"Brief generation cost: "
        f"${brief.estimated_cost:.6f}"
    )

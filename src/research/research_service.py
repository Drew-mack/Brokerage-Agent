...
from brief.email_renderer import EmailRenderer
from mailer.ses_sender import SESEmailSender
from storage.delivery_storage import (
    get_last_delivered_session,
    record_delivery,
)
from storage.thesis_storage import (
    ThesisStorageError,
    get_latest_thesis,
    save_thesis,
)
from brief.morning_brief import (
    MorningBriefBuilder,
    print_morning_brief,
)
from research.finnhub_news import (
    FinnhubError,
    FinnhubNewsClient,
    NewsArticle,
)
from research.forward_analyzer import (
    ForwardAnalysis,
    ForwardAnalyzer,
    ForwardAnalyzerError,
)
from research.portfolio_advisor import (
    PortfolioAdvisor,
...
def print_movement_research(
    results: list[PositionResearch],
):
    """Print movement research for development verification."""

    print()
    print("=" * 65)
    print(
        "PORTFOLIO MOVEMENT RESEARCH"
    )
    print("=" * 65)

    if not results:
        print(
            "\nNo positions met the movement "
            "research threshold."
        )
        return
    ...
    for result in results:
        ...
        print(
            f"Dollar Change:          "
            f"${result.dollar_change:+,.2f}"
        )

        print(
            f"Portfolio Contribution: "
            f"{result.portfolio_contribution:+.2%}"
        )
        ...
        print(
            f"OpenAI Cost: "
            f"${analysis.estimated_cost:.6f}"
        )


def print_forward_research(
    results: list[ForwardPositionResearch],
):
    """Print forward research and portfolio advice."""

    print()
    print("=" * 65)
    print(
        "PORTFOLIO FORWARD RESEARCH"
    )
    print("=" * 65)

    if not results:
        print(
            "\nNo positions met the forward "
            "research threshold."
        )
        return
    ...
    for result in results:
        ...
        print(
            f"Advice Cost:   "
            f"${result.advice.estimated_cost:.6f}"
        )


def print_total_cost(
    movement_results: list[PositionResearch],
    forward_results: list[ForwardPositionResearch],
):
    """Print total LLM cost for the research run."""

    movement_cost = sum(
        result.analysis.estimated_cost
        for result
        in movement_results
    )
...
def run_morning_brief(
    ...
    print(
        "SUCCESS: Portfolio analytics generated."
    )

    print(
        "Completed session: "
        f"{analytics.session_date}"
    )
    ...
    if (
        ...
        if (
            ...
            print(
                "SKIPPED: Morning Brief "
                f"for {session_date} was "
                "already delivered."
            )
    ...
    forward_results = (
        service.research_portfolio_forward(
            analytics=analytics,
            movement_results=(
                movement_results
            ),
    ...
    if send_email:
        sender_email = (
            os.getenv(
                "PORTFOLIO_SENDER_EMAIL",
                "drewmack04@icloud.com",
            )
        )

        recipient_email = (
            os.getenv(
                "PORTFOLIO_RECIPIENT_EMAIL",
                "drewmack04@icloud.com",
            )
        )
        ...
        print(
            "SUCCESS: Morning Brief email sent."
        )
        ...
        if prevent_duplicate_delivery:
            ...
            print(
                "SUCCESS: Delivery state recorded "
                f"for {session_date}."
            )
